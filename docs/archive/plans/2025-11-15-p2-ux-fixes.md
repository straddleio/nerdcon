...
describe('Geolocation Proxy Endpoint', () => {
  ...
  it('should return mock data for private IPs', async () => {
    const response = await request(app).get('/api/geolocation/192.168.1.1').expect(200);
  ...
  it('should handle geolocation service errors gracefully', async () => {
    const response = await request(app).get('/api/geolocation/invalid-ip').expect(200);
...
The existing `formatText` function should already handle bullets (`-`, `*`, `•`). Verify it's being called:

```tsx
// web/src/components/Terminal.tsx - check formatText function exists
// Should detect lines starting with '- ' or '* ' or '• '
...
3. **Issue #9**: Inspect balance display code → verify cents documented
4. **Issue #11**: Type `/outcomes` → verify it works
5. **Issue #13**: Type `/create-cu` + Tab → autocomplete works
...
✅ **Issue #16** - Tab autocomplete fails
...
- ✅ Removed outdated testing instruction references
